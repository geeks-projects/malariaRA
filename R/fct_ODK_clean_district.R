...
district_data_cleaner <- function(data){
  data |>
...
district_summary_fn <- function(assessment_period =  c( "3m", "2m", "1m", "2wk", "1wk")) {
...
ODKtoDB_district_essential_clean <- ODKtoDB_district |>
  dplyr::select(-c(c(starttime:gps1) , c(level_supervision:other_agency), c(level_supervision:planning_coordination),
                   c(training,m_e,cold_chain_and_logistics, district_level_on_track_intro_malaria,overall_comments, advocacy_communication, thank_you),
...
district_essential_3m <- ODKtoDB_district_essential_clean |>
  ...
  mutate(case_when(
    time_of_assessment == "3m" ~ across(!c(time_of_assessment, ends_with(district_activities_3m)), ~ NA)
  ))
...
district_essential_2m <- ODKtoDB_district_essential_clean |>
  ...
  mutate(case_when(
    time_of_assessment == "2m" ~ across(!c(time_of_assessment, ends_with(district_activities_2m)), ~ NA)
  ))

district_essential_1m <- ODKtoDB_district_essential_clean |>
  filter( time_of_assessment == "1m" ) |>
  mutate(case_when(
    time_of_assessment == "1m" ~ across(!c(time_of_assessment, ends_with(district_activities_1m)), ~ NA)
  ))

district_essential_2wk <- ODKtoDB_district_essential_clean |>
  filter( time_of_assessment == "2wk" ) |>
  mutate(case_when(
    time_of_assessment == "2wk" ~ across(!c(time_of_assessment, ends_with(district_activities_2wk)), ~ NA)
  ))
...
### district detail. ##########
district_detail <- district_essential |>
  district_data_cleaner() |>
...
district_summary <- switch(
  ...
  "3m"      = {
    district_summary_fn(assessment_period = "3m") |>
